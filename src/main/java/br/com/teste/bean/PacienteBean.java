...
public class PacienteBean implements Serializable {
    ...
    /**
     * Verifica se a exceção indica violação de chave estrangeira para RECEITA → PACIENTE
     * (PostgreSQL SQLState 23503 ou constraint fk_receita_paciente).
     */
    private boolean isForeignKeyViolationToReceita(Throwable t) {
        if (t instanceof org.hibernate.exception.ConstraintViolationException) {
            org.hibernate.exception.ConstraintViolationException cve = (org.hibernate.exception.ConstraintViolationException) t;
            String sqlState = cve.getSQLException() != null ? cve.getSQLException().getSQLState() : null;
            String constraintName = cve.getConstraintName();
            if ("23503".equals(sqlState)) return true; // Violação de FK (PostgreSQL)
            if (constraintName != null && constraintName.equalsIgnoreCase("fk_receita_paciente")) return true;
            String msg = cve.getMessage();
            if (msg != null && msg.toLowerCase().contains("fk_receita_paciente")) return true;
        }
        if (t instanceof java.sql.SQLIntegrityConstraintViolationException) return true;
        if (t instanceof java.sql.SQLException) {
            String sqlState = ((java.sql.SQLException) t).getSQLState();
    ...
    public void excluir(Long id) {
        try {
            ...
            Throwable root = getRootCause(e);
            String msg;
            if (isForeignKeyViolationToReceita(root)) {
                msg = "Não é possível excluir paciente vinculado a uma receita.";
            } else {
                msg = "Erro ao excluir paciente: " + (root != null && root.getMessage() != null ? root.getMessage() : e.getClass().getSimpleName());
            }
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, msg, null));
        }
